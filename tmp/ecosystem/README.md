...
**Tool discovery boundary**: Two documents discuss tool discovery. Plugin/Extension covers local discovery from configured manifest and WASM sources. MCP covers remote protocol discovery through the `tools/list` JSON-RPC handshake over HTTP/stdio/socket to external servers. Again complementary; see [Boundary Clarification](#boundary-clarification).
...
|----------|---------|----------|
| [On-Chain Reputation](./chain-reputation/) | Candidate identity passports, domain-specific reputation, graph-trust analysis, bounty marketplace mechanics, HTTP payment patterns, and token-policy questions. Relevant for NEAR integration, but all constants and economics remain validation targets. Split into: passport-system, reputation-scoring, bounty-marketplace, token-economics, near-implementation, benchmarking. | MEDIUM |
...
| [Smart Contract Architecture](./smart-contracts/README.md) | NEAR contract sketches for agent registration, worker bonds, reputation inputs, bounty escrow, and optional public content hashes. Start small and benchmark each deployed contract before expanding. Split into: [near-contracts](./smart-contracts/near-contracts.md), [ironclaw-integration](./smart-contracts/ironclaw-integration.md), [references](./smart-contracts/references.md). | MEDIUM |
...
| **Audience** | Operators, dashboards, CI scripts | Editors, agents, and tool clients |
...
|--|-----------------|-----|
| **Discovery mechanism** | Configured local manifest/WASM scan | JSON-RPC `tools/list` request over a configured transport |
| **Where tools live** | Local configured catalogs and packaged extensions | External or local MCP servers |
...
**Scope note:** Plugin/Extension and Control Plane cover captured extensibility and operations patterns. ACP/MCP covers editor-facing and tool-facing protocols. On-Chain Reputation and Smart Contracts cover candidate economic trust inputs. All five connect at the boundary where IronClaw interacts with external systems (editors, blockchains, operator dashboards).
